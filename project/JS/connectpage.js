...
function countnumber(targetElement, start, end, duration) {
  //숫자 카운트 애니메이션
  let startTime = null;
...
function getTranslateX(card) {
  const style = window.getComputedStyle(card); // 카드의 현재 스타일 가져오기
  const matrix = new DOMMatrix(style.transform); // transform 속성의 행렬 변환
  return matrix.m41 || 0; // 행렬에서 X 변환 값 반환
}
//btn에도 적용
